...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if text == '/start':
            ...
            # Отправляем приветствие с Reply Keyboard и кнопкой меню
            reply_keyboard = {
                'keyboard': [
                    [{'text': '☰ Меню'}]
                ],
            ...
            send_telegram_message(
                ...
                '📚 Нажми кнопку "☰ Меню" внизу, чтобы выбрать режим обучения',
                reply_keyboard,
                parse_mode=None
            )
        
        # Кнопка "☰ Меню" - выбор режима обучения
        elif text == '☰ Меню':
            keyboard = get_learning_mode_keyboard()
            send_telegram_message(
                chat_id,
                '🎓 Выбери режим обучения:\n\n'
                '💬 Диалог - свободное общение с Аней. Практикуй разговорную речь в естественных диалогах\n\n'
                '✍️ Предложения - составляй предложения с изученными словами. Развивай навык построения фраз\n\n'
                '📝 Контекст - вставляй пропущенные слова в предложения. Учись понимать слова в контексте\n\n'
                '🎯 Ассоциации - угадывай слова по трём подсказкам. Тренируй словарный запас через ассоциативное мышление\n\n'
                '🇷🇺→🇬🇧 Перевод - переводи слова с русского на английский. Закрепляй знание лексики',
                keyboard,
                parse_mode=None
            )
        else:
